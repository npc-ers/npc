...
contract FourFourFour is ERC721, Ownable {
    ...
    uint256 public constant premintPrice = 80000000000000000; // 0.08 ETH
    uint256 public constant salePrice = 100000000000000000; // 0.10 ETH

    uint256 public constant presaleLimit = 444; 
    uint256 public constant saleLimit = 4444; 

    uint256 public constant maxNumPurchase = 15; 
    ...
    function flipStoreIsOpen() public onlyOwner {
    ...
    function presaleMint(uint256 numberOfTokens) public payable {
        require(storeIsOpen, "Store must be open!");
        require(presaleIsActive, "Preminting must be active!");
        require(
            totalSupply().add(numberOfTokens) <= presaleLimit,
            "Purchase would exceed presale supply"
        );
        require(
            premintPrice.mul(numberOfTokens) <= msg.value,
            "Ether value sent is not correct"
        );
        
        for (uint256 i = 0; i < numberOfTokens; i++) {
            uint256 mintIndex = totalSupply();
            if (totalSupply() < presaleLimit) {
                _safeMint(msg.sender, mintIndex);
            }
        }
    }

    function mainSaleMint(uint256 numberOfTokens) public payable {
        require(storeIsOpen, "Store must be open!");
        require(saleIsActive, "Sale must be active!");
        require(
            totalSupply().add(numberOfTokens) <= saleLimit,
            "Purchase would exceed max supply"
        );
        require(
            numberOfTokens < saleLimit,
            "Number purchase exceeds max in one transaction"
        );
        require(
            salePrice.mul(numberOfTokens) <= msg.value,
            "Ether value sent is not correct"
        );

        if (whitelistEnabled) {
            require(verifyUser(msg.sender), 'User must be on whitelist');
        }
