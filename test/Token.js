...
// an async function.
describe("FourFourFour contract", function () {
  ...
  beforeEach(async function () {
    ...
    FourContract = await ethers.getContractFactory("FourTest");
    [owner, addr1, addr2, ...addrs] = await ethers.getSigners();

    // To deploy our contract, we just have to call Token.deploy() and await
    // for it to be deployed(), which happens onces its transaction has been
    // mined.
    hardhatContract = await FourContract.deploy();
  ...
  describe("Deployment", function () {
    ...
    it("Should assign the total supply of tokens to the owner", async function () {
      const ownerBalance = await hardhatContract.balanceOf(owner.address);
      expect(await hardhatContract.totalSupply()).to.equal(ownerBalance);
    });
  });
